/**
 * Compliant Solution (using atomic increment operator)
 * This solution uses the atomic increment operator (++)
 * which performs the increment operation atomically.
 * 
 * When applied to an atomic variable, the ++ operator
 * is guaranteed to be atomic, equivalent to atomic_fetch_add(&counter, 1).
 **/
...
/**
 * Increments the counter atomically using the ++ operator.
...
int main(void) {
  ...
  if (final_counter == expected_counter) {
    ...
    printf("The atomic ++ operator prevents race conditions.\n");
    printf("All %d increments were successfully applied.\n", expected_counter);
  } else {
    printf("\n⚠️  ERROR: Unexpected result!\n");
    printf("This should not happen with atomic ++.\n");
  }
